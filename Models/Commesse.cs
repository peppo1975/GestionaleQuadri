    ...
    public class Commesse : Generale
    {
        ...
        private static List<CommesseTable> CommesseTable(List<CommesseAzienda> commesseAzienda)
        {
            ...
            foreach (InfoCommessa item in commesse)
            {
                ...
                List<InfoQuadri> quadri = commesseAzienda.Where(x => x.nome_commessa == item.nome_commessa).Select( p => new InfoQuadri {nome_quadro = p.nome_quadro, data_inserimento = p.data_inserimento}).ToList();
